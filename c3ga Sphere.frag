...
// Adapted from ganja : https://github.com/enkimute/ganja.js/blob/master/codegen/cpp/cga.cpp
...
void main()
{
    ...
    // Ray Tracing
    Mvec rayLine = outer(outer(ro, rd), ei);                          // rayLine = ro ^ rd ^ ei
    Mvec intersectionPtPair = dual(outer(dualSphere, dual(rayLine))); // intersectionPtPair = !(dualSphere ^ !rayLine)
		// Normal
		Mvec p1, p2;
